...
export const getUser_api = async (req: Request, res: Response) => {
  ...
  try {
    const [users] = await dbcon.execute<UserAllGetRes[]>(
      `SELECT U.USER_ID, U.USERNAME, U.EMAIL, U.PHONE_NUMBER, U.ROLE_TYPE_ID, U.ACCOUNT_STATUS, DO.FIRST_NAME, DO.LAST_NAME, DO.PROFILE_IMAGE, DO.FACEBOOK, DO.LINE, DO.INSTAGRAM, DO.TELEGRAM, DO.X, DO.REQ_STATUS FROM USERS U LEFT JOIN DORM_OWNERS DO ON U.USER_ID = DO.USER_ID WHERE U.USER_ID = ?`,
      [id.toString().trim()],
    );
    if (users.length > 0) {
...
export const getMyFavorites_api = async (req: Request, res: Response) => {
  try {
    ...
    const sql = `
            ...
            WHERE F.USER_ID = ?
            GROUP BY F.FAV_ID, D.DORM_ID, D.DORM_NAME, OWNERNAME, D.UPDATE_AT, D.ADDRESS, D.FRONT_DORM_IMAGE, D.SCORE, DS.DORM_STATUS_NAME, DZ.ZONE_NAME
